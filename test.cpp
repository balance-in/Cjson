...
#define EXPECT_EQ_DOUBLE(expect, actual) EXPECT_EQ_BASE((expect) == (actual), expect, actual, "%1.7g")
...
static void test_parse_array(){
    ...
    for (int i=0; i<4; i++){
        lept_value *it = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(it));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(it));
        for (int j=0; j<i; j++){
            lept_value *iter = lept_get_array_element(it, j);
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(iter));
            EXPECT_EQ_DOUBLE((double)j, lept_get_number(iter));
        }
    }
    lept_free(&v);
}
